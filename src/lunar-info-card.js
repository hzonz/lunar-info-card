...
export class LunarInfoCard extends LitElement {
  static properties = { hass: {}, config: {}, _data: {} };
  ...
  static styles = css`
    ...
    ha-card { padding:10px; box-sizing:border-box; height:100%; }
    .card-container { display:grid; grid-template-areas:"a b b b" "a c c c" "a d d d" "a e f g" "h h h h" "i i i i" "j j j j" "k k k k"; grid-template-columns:repeat(4,1fr); gap:2px; }
    .lunar { grid-area:a; display:grid; grid-template-areas:"n l"; }
    .lunar .name { writing-mode:vertical-lr; font-weight:bold; font-size:2em; }
    .lunar .label { writing-mode:vertical-lr; font-size:1em; }
    .label, .label-block { font-size:0.9em; }
  ...
  get fields() {
    return this.config.customize && this.config.fields ? { ...DEFAULT_FIELDS, ...this.config.fields } : DEFAULT_FIELDS;
  }

  getGridOptions() {
    return { rows: 8, min_rows: 8, columns: 12, min_columns: 9, max_columns: 12 };
  }
  ...
  updated() {
    if (this.hass && this.config) {
      ...
      this._data = Object.fromEntries(Object.entries(this.fields).map(([k, t]) => [k, evalTemplate(t, states, entity)]));
    }
  }
  ...
  _section(area,name,value,layout="a",bold=false,circle=null,circleColor=null) {
    ...
    if(area==="a"&&name==="农历"){
      return html`<div class="lunar" style="grid-area:${area};">
        ...
        <div class="label">${(value?.ganzhiYear||"")+" "+(value?.ganzhiMonth||"")+" "+(value?.ganzhiDay||"")+" "+(value?.week||"")}</div>
      </div>`;
    }
    return html`<div class="${cls}" style="grid-area:${area};">
      ${circle?html`<div class="circle ${circleColor||''}">${circle}</div>`:html`<div class="name" style="${bold?'font-weight:bold':''}">${name}</div>`}
      <div class="${circle?'label-block':'label'}">${value||""}</div>
    </div>`;
  }

  static getStubConfig() { return { entity:"sensor.nong_li" }; }
  static async getConfigElement() { await import("./lunar-info-card-editor.js"); return document.createElement("lunar-info-card-editor"); }
}
